...
[Click here to finish setting up your workspace!](https://cloud.nx.app/connect/rBNDLIFZC2)
...
```sh
npx nx serve ChuanitosLab
```

To create a production bundle:

```sh
npx nx build ChuanitosLab
```

To see all available targets to run for a project, run:

```sh
npx nx show project ChuanitosLab
```
...
targeted file structure looks like this
```
libs/
├── core/
│   └── services/         <-- Global services like AuthService, API clients
├── shared/
│   ├── ui/               <-- Reusable UI components (buttons, cards, etc.)
│   └── utils/            <-- Helper functions, pipes, validators
├── users/
│   ├── data-access/      <-- User-specific services: fetching user data, handling API calls (UserService)
│   ├── ui/               <-- User-specific UI components (ProfileCard, UserForm, etc.)
│   └── feature/          <-- Smart components like UserProfilePage, UserListPage
```
